...
async function seed() {
  try {
    ...
    await Transaction.insertMany(
      transactions.map((t) => ({ userId: user._id, status: 'completed', accountType: 'savings', ...t }))
    );
    ...
    await Alert.insertMany([
      { userId: user._id, type: 'Fraud',    severity: 'high',   read: false, message: 'Unusual login attempt detected from a new device in Delhi. Please verify.',        ts: '2h ago' },
      { userId: user._id, type: 'Spending', severity: 'medium', read: false, message: 'You have spent ₹8,500 on Entertainment this month — 42% above your average.',      ts: '5h ago' },
      { userId: user._id, type: 'Tips',     severity: 'low',    read: true,  message: 'Tip: Enable biometric authentication for faster and safer login.',                  ts: '1d ago' },
      { userId: user._id, type: 'Fraud',    severity: 'medium', read: true,  message: 'Your card was used in a contactless transaction at BigBazaar, Mumbai.',             ts: '2d ago' },
      { userId: user._id, type: 'Spending', severity: 'low',    read: true,  message: 'Monthly spending summary: ₹47,549 spent in March 2026.',                           ts: '3d ago' },
    ]);
